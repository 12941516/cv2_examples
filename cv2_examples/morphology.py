...
class Morphology(Node):
    def __init__(self):
        super().__init__('morphology_node')
    ...
    def redHsvInrange(self, src):
        ...
        return cv2.bitwise_or(hsv_dst, hsv_dst2)
    
    # Morphology Filter Added
    def morph_dilate(self, src):
        dilated_src = cv2.dilate(src, self.morph_kernel, iterations=1)
        return dilated_src
    ...
    def videoSubscriber_callback(self, msg):
        ...
        dilated_src = self.morph_dilate(hsv_red_src)
        cv2.imshow('gaussian', gaussian_src)
        cv2.imshow('hsv', hsv_red_src)
        cv2.imshow('morph', dilated_src)
